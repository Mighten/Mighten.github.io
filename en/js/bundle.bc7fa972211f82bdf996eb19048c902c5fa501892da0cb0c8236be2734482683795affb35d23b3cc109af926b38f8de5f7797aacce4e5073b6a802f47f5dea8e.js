...
const baseURL = 'https://mighten.github.io/';
...
function renderMermaidBeforePreCodeWrapped() {
...
/**
 * preprocessing <pre class="language-plantuml"> before it is wrapped up with a code fence
 */
function renderPlantUmlBeforePreCodeWrapped() {
    Array.prototype.forEach.call(
        ...
        function(code){
            ...
            // replace <pre> tag with <div> tag, before it is wrapped up with a ugly code fence (`highlight_wrapper`)
            code.parentElement.outerHTML = `<div class="plantuml-container">${image.outerHTML}</div>`;
        }
    );
}
...
renderPlantUmlBeforePreCodeWrapped();
renderMermaidBeforePreCodeWrapped();
  ...
  function populateAlt(images) {
    ...
    images.forEach((image) => {
      ...
      if (addCaption) {
        ...
        // insert caption before image -- modified by Mighten <mighten@outlook.com>
        // this may not be beautiful: image.insertAdjacentHTML('beforebegin', desc.outerHTML);
      }
